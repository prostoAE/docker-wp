...
$fields_name = !empty($args['field_name']) ? $args['field_name'] : 'letsmake_settings_general';
...
<div class="letsmake-wrapp">
    ...
    <h2>Lets make General Settings</h2>
    <?php echo '<pre>';
    var_dump($options);
    echo '</pre>';?>
    <hr><br>
    <form action="/wp-admin/admin.php?page=letsmake_settings" method="post">
        <div class="letsmake-wrapp__form-body">
            ...
            if (is_array($args_display)) {
                foreach ($args_display as $item) {
                    ...
                    }elseif('select' === $item['html']) {

                        $name = !empty($item['name']) ? esc_attr($item['name']) : '';
     
                        $default = !empty($item['default']) ? esc_attr($item['default']) : '';
                        $value = !empty($options[$name]) ? esc_attr($options[$name]) : $default;
                        $ltm_options->html_select($fields_name, $item, $value);
                    }
                }
            }
            ?>
